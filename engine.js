...
//Particle System ---------------------------------------------------------------------------------------------------------------//
function ship_debris() {
    this.stage = 4;
    this.debris = new Set();

    //takes in a parts array of already randomized elements.  Keep in mind that parts array length may need to match debris length
    this.init = function(parts_array) {
        this.debris.clear();
        for (i = 0; i < (this.stage * 3); i++) {
            this.debris.add(parts_array[i]);
        }
    };
    this.change_stage = function(num) {
        this.stage = num;
    };
    this.update = function(speed) {
        for (let part of debris) {
						part.Y += speed;
        }
    }
}
